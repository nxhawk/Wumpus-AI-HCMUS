import copy
from Algorithm.Cell import Cell, Object
from Algorithm.KnowledgeBase import KnowledgeBase
from Algorithm.Action import Action
...
class Solution:
    ...
    def read_map(self, map_filename):
        ...
        for ir in range(self.map_size):
            for ic in range(self.map_size):
                self.cell_matrix[ir][ic] = Cell((ir, ic), self.map_size, raw_map[ir][ic])
                if Object.AGENT.value in raw_map[ir][ic]:
        ...
        if not result:
            ...
            raise TypeError(
                'Input Error: The map is invalid! Please check at row ' + str(pos[0]) + ' and column ' + str(
                    pos[1]) + '.')
    ...
    def add_new_percepts_to_KB(self, cell):
        ...
        if cell.exist_pit():
            sign = '+'
            self.KB.add_clause([cell.get_literal(Object.WUMPUS, '-')])
        ...
        if cell.exist_wumpus():
            sign = '+'
            self.KB.add_clause([cell.get_literal(Object.PIT, '-')])
